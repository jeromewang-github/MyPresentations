function [model , Y] = MFA_learn(data, gnd, k, metric, kernel, maxbits,lambda)
...
if bGlobal & isfield(options,'keepMean') & options.keepMean
    [eigvector, eigvalue] = LGE(Sp, Sc, options);
else
    [eigvector, eigvalue] = LGE(Sp,Sc,options,data,maxbits);
end
...
model.P = eigvector;
Y = MFA_compress(data, model, kernel, maxbits);
