function [model , Y] = LSDAm_learn(feaTrain, gnd, k, metric, kernel, maxbits,lambda)
...
%     data = repmat(DToPowerHalf,1,nFea).*data;
    [Y, eigvalue] = LGE(W, [], options, data,maxbits);
else
	data=eye(nSmp);
    Dw = sparse(1:nSmp,1:nSmp,Dw,nSmp,nSmp);
    
    [Y, eigvalue] = LGE(W, Dw, options, data,maxbits);
end

eigIdx = find(eigvalue < 1e-10);
eigvalue (eigIdx) = [];
Y=Y(:,1:min(maxbits,length(eigvalue)));
...
%         model(p) = {svmtrain([],B(:,p),X,'-q')};
         model(p) = {svmtrain([],B(:,p),data, '-t 0')};
    end
end
if strcmp(kernel,'Gaussian')
    for p = 1:maxbits
        model(p) = {svmtrain([],B(:,p),data,['-q -c 1 -g ',int2str(length(X))])};
    end
end
