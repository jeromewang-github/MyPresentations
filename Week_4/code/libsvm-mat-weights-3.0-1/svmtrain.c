...
{
	mexPrintf(
	"Usage: model = svmtrain(weight_vector, training_label_vector, training_instance_matrix, 'libsvm_options');\n"
	"libsvm_options:\n"
...
{
	int i, j, k;
	int elements, max_index, sc, label_vector_row_num, weight_vector_row_num;
		...
		int nrhs, const mxArray *prhs[] )
{
	...
	if(nrhs > 2 && nrhs < 5)
	{
		...
		if(!mxIsDouble(prhs[0]) || !mxIsDouble(prhs[1]) || !mxIsDouble(prhs[2])) {
			mexPrintf("Error: wieght vector, label vector and instance matrix must be double\n");
			fake_answer(plhs);
			return;
		}
